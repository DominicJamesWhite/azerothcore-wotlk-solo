...
def _comment_block(text):
    """Split `text` into lines, ensuring EVERY line stays a SQL comment.

    Spell description/tooltip values contain embedded newlines, so a naive
    f"-- {text}" only comments the first line and leaks the remainder into
...
def generate_dbc_sql(spell_id, row, field_names, comment, overrides, old_values):
    ...
    # Header comment
    if comment:
        lines.extend(_comment_block(f"-- {comment}"))
    ...
    if overrides:
        ...
        # Spell text columns routinely contain embedded newlines (e.g. the
        # "Conjured items disappear..." line on every Create Soulstone rank).
        # Without re-prefixing, only the first line stays commented and the
        # rest is parsed as SQL, which aborts the whole world DB update.
        overrides_comment = f"-- Overrides: {', '.join(diff_parts)}"
        lines.extend(_comment_block(overrides_comment))
